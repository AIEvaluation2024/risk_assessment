...
for i in range(1,20,1):
    n_value =int(n/i)
    print('solving for n: ', n_value)
    values,methods = RA_simulation_routines.iid(model_type,data_type,num_classes,data_boundary,n_value,base_file_model,score_type)
    filename_error_table = "results/error_table_" + data_type + "_" + model_type + "_without_" + "n" + str(n_value)  + ".csv"
    utils.write_results([values],methods,filename_error_table)
...
for i in range(1,20,1): 
    n_value = int(n/i)
    print('solving for n: ',n_value)
    values,methods = RA_simulation_routines.covariate_shift(model_type,data_type,num_classes,data_boundary,n_value,base_file_model,score_type,beta_data)
    filename_error_table = "results/error_table_" + data_type + "_" + model_type + "_with_" + "n" + str(n_value)  + ".csv"
    utils.write_results([values],methods,filename_error_table)
